function [sol,b,obj,sv] = primal_svm(linear,Y,lambda,opt)
...
% Last modified 25/08/2006  
  ...
  if  linear 
    ...
    if ~opt.cg
      [sol,obj,sv] = primal_svm_linear   (Y,lambda,opt);
    else
      [sol,obj,sv] = primal_svm_linear_cg(Y,lambda,opt);
    end;
...
function  [w,obj,sv] = primal_svm_linear(Y,lambda,opt) 
% -------------------------------
...
function  [w, obj, sv] = primal_svm_linear_cg(Y,lambda,opt)
% -----------------------------------------------------
...
% -----------------------------------------------------
  ...
  while 1
    ...
    [obj, gn, sv] = obj_fun_linear(w,Y,lambda,out); gn=-gn;
